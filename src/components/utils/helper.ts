...
export function replaceObjects<T extends Record<string, any>>(original: T[], replacements: T[], keyId: string) {
	return original.map((originalObj) => {
		// Find a replacement object with the same ID
		const replacementObj = replacements.find((replacement) => replacement[keyId] === originalObj[keyId]);
...
export function copyObjectsWithUniqueId<T extends Record<string, any>>(
	sourceArray: T[],
	destinationArray: T[],
	keyId: string,
...
export const wordVariation = (count: number, word: string, hideCount?: boolean) => {
...
export const handleScrollRightSection = (ref: React.RefObject<HTMLDivElement>) => {
	if (ref.current) {
...
export function concatenateUnique<T extends ObjectWithId>(arr1: T[], arr2: T[], idKey: keyof T): T[] {
