...
const NotificationModal: React.FC<INotificationModal> = ({ useNotificationProps, size, children, disableClose }) => {
	...
	const matchChild: any = React.Children.map(children, (child) => {
		if (child) return { ...child, props: { ...child.props, useNotificationProps } };
		return child;
	});
	...
	return (
		<Modal
			tabIndex={-1}
			show={openModal}
			className={size === 'wide' ? 'wide-modal' : size === 'medium' ? 'medium-modal' : ''}
		>
			<Modal.Dialog
